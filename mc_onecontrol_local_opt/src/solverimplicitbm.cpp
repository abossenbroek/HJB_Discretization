...
SolverImplicitBM::solve()
{
  ...
  gsl_vector_set_all(b, 1.0);
    ...
    for (policy_idx = 0; policy_idx < p->policy_size; policy_idx++) {
      ...
      // Add the drift only at a later stage.
      ...
      __m128d packed_zeros = _mm_set1_pd(0);
      __m128d packed_minones = _mm_set1_pd(-1);
      ...
      while (prob_sub_diag_elm != prob_sub_diag_end) {
        ...
        //
        __m128d packed_sub_res_int = _mm_min_pd(packed_drift0, packed_zeros);
        // 
        // First step for the upwards probability.
        //
        __m128d packed_sup_res = _mm_max_pd(packed_drift1, packed_zeros);
        
        // Finish the operator split operation.
        __m128d packed_sub_res = _mm_mul_pd(packed_sub_res_int, packed_minones);
      ...
      double *_drift_elm = drift->data;
      double *_drift_end = drift->data + drift->size;
      double *_prob_curr_state_vec_elm = prob_curr_state_vec->data;

      // Add the drift element to the probability that the process remains in
      // the current state. This is the last step required in the
      // computations.
      while (_drift_elm != _drift_end) {
        (*_prob_curr_state_vec_elm) += fabs(*_drift_elm);
        _drift_elm++;
        _prob_curr_state_vec_elm++;
      } 
